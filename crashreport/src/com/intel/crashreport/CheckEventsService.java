...
public class CheckEventsService extends Service {
	...
	private void stopService() {
		...
		if(app.getRequestListCount() > 0 && !app.isServiceRelaunched()) {
			...
			timer.schedule(new RelaunchServiceTask(getApplicationContext()), CrashReport.CRASH_POSTPONE_DELAY*1000);
		}
		stopSelf();
	}
								...
								if (result) {
									...
									if (ret == -1)
										...
										if (!event.isDataReady()) {
											...
											timer.schedule(new NotifyCrashTask(event.getEventId(),getApplicationContext()), CrashReport.CRASH_POSTPONE_DELAY*1000);
										}
									}
								}
							} else
